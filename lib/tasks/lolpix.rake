...
namespace :lolpix do
  desc 'Configure the server for production use'
  task :init do
    prompt = TTY::Prompt.new
    env = {}

    begin
      prompt.say('Warning: Changing the domain afterwards can have negative side effects!')
      ...
      env['LOLPIX_PORT'] = prompt.ask('Server port:') do |q|
        q.required true
        q.default 443
        q.convert :int
      end
      ...
      loop do
        ...
        begin
          ...
          redis.ping
          prompt.ok 'Redis connection successful!'
          break
        rescue StandardError => e
          prompt.error 'Redis connection failed!'
          prompt.error e.message
          break unless prompt.yes?('Try again?')
        end
      end
      ...
      loop do
        if prompt.yes?('Do you want to send e-mails from localhost?', default: false)
          ...
          allowed_modes = %w[none peer client_once fail_if_no_peer_cert]
          env['SMTP_OPENSSL_VERIFY_MODE'] = prompt.select('SMTP OpenSSL verify mode:', allowed_modes)
        end
        ...
        begin
          ...
          prompt.error 'Sending mail failed!'
          prompt.error e.message
          break unless prompt.yes?('Try again?')
        end
      end
      ...
      prompt.say "\n"
      prompt.say 'This configuration will be written to .env'
      ...
      if prompt.yes?('Save configuration?')
        ...
        File.write(Rails.root.join('.env'), "# Generated with lolpix:init on #{Time.now.utc}\n\n#{env_contents}\n")
        ...
        if prompt.yes?('Prepare the database now?')
          ...
          merged_values = env.transform_values(&:to_s).merge({ 'RAILS_ENV' => 'production', 'SAFETY_ASSURED' => '1' })
          if !system(merged_values, 'rails db:setup')
        ...
        prompt.say "\n"
        prompt.ok 'All done! You can now start the "lolpix-web" service!'
        prompt.say "\n"
